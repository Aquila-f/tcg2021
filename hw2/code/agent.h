...
public:
	...
	virtual action take_action(const board& before) {
		...
		for (int op : {0, 1, 2, 3}){
			...
			// for(int op1 : {0, 1, 2, 3}){
			// 	board afterafter = after;
			// 	int reward1 = afterafter.slide(op1);
			// 	// std::cout << after << std::endl;
			// 	// std::cout << op1 << std::endl;
			// 	// std::cout << reward1 << std::endl;
			// 	if(reward1 == -1) continue;
			// 	// exit(0);
			// 	float value1 = estimate_value(afterafter);

			// 	if (reward + reward1 + value1 > best_reward + best_reward1 + best_value1){
			// 		best_op = op;
			// 		best_reward = reward;
	...
	virtual void close_episode(const std::string& flag = "") {
		...
		for(int t = history.size()-2; t>=0; t--){
			...
			// adjust_value(history[t].after, history[t+1].reward, history[t+1].after);
		}
	}
	...
	float estimate_value(const board& after) const {
		...
		for(int i=0;i<2;i++){
		for(int j=0;j<4;j++){
			value += net[0][extract_feature5(temp_board, 0, 1, 2, 3, 4)];
			value += net[1][extract_feature5(temp_board, 4, 5, 6, 7, 8)];
			value += net[2][extract_feature5(temp_board, 0, 1, 2, 4, 5)];
			value += net[3][extract_feature5(temp_board, 4, 5, 6, 8, 9)];

			// value += net[4][extract_feature(temp_board, 0, 4, 8, 12)];
			// value += net[5][extract_feature(temp_board, 1, 5, 9, 13)];
			// value += net[6][extract_feature(temp_board, 2, 6, 10, 14)];
			// value += net[7][extract_feature(temp_board, 3, 7, 11, 15)];
	...
	void adjust_value(const board& after, float target){
		board tempboard_after = after;
		float current = estimate_value(tempboard_after);
		float adjust, error;
		...
		for(int j=0;j<4;j++){
			...
			adjust = alpha * error;
			net[0][extract_feature5(tempboard_after, 0, 1, 2, 3, 4)] += adjust;
			net[1][extract_feature5(tempboard_after, 4, 5, 6, 7, 8)] += adjust;
			net[2][extract_feature5(tempboard_after, 0, 1, 2, 4, 5)] += adjust;
			net[3][extract_feature5(tempboard_after, 4, 5, 6, 8, 9)] += adjust;
			// net[4][extract_feature(tempboard_after, 0, 4, 8, 12)] += adjust;
			// net[5][extract_feature(tempboard_after, 1, 5, 9, 13)] += adjust;
			// net[6][extract_feature(tempboard_after, 2, 6, 10, 14)] += adjust;
			// net[7][extract_feature(tempboard_after, 3, 7, 11, 15)] += adjust;
			tempboard_after.rotate_right();
		}
		tempboard_after.reflect_horizontal();}
